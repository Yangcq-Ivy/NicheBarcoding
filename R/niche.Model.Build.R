                               ...
                               en.vir = NULL, bak.vir = NULL)
{
  search.For.Diff.Absen.From.Prese <- function(prese, absen) {
    ...
    dist2center.prese <- apply(prese, 1, eucl.dist.two.vect,
                               v2 = group.mean.prese)
    ci95 <- stats::quantile(dist2center.prese, prob = c(0.025,
                                                        0.975), na.rm = T)
    dist2center.absen <- apply(absen, 1, eucl.dist.two.vect,
                               v2 = group.mean.prese)
    dist2center.absen
  ...
  if (is.null(prese) == TRUE & is.null(prese.env) == FALSE) {
    ...
    if (!is.data.frame(prese) | dim(prese)[2] != 3) {
      ...
      if (is.null(prese.env) == TRUE) {
        if (nrow(prese) < 10) {
          prese <- pseudo.present.points(prese, 10, 10,
                                         2, en.vir = en.vir)
        }
        present.env0 <- raster::extract(en.vir, prese[,
                                                      2:3])
      }else {
        present.env0 <- prese.env
        if (nrow(present.env0) < 3) {
  ...
  if (is.null(absen) == TRUE & is.null(absen.env) == TRUE) {
    ...
    if (is.null(bak.vir) == TRUE) {
      back <- dismo::randomPoints(mask = en.vir, n = outputNum *
                                    2, ext = NULL, extf = 1.1, excludep = TRUE, prob = FALSE,
      ...
      samp <- sample(dim(diff)[1], size = outputNum, replace = T)
      absent.env0 <- diff[samp, ]
    }else {
      bak0 <- bak.vir[, colnames(bak.vir) %in% colnames(present.env0)]
      if (nrow(bak.vir) < outputNum * 2) {
        more <- outputNum * 2 - nrow(bak.vir)
        more.back <- dismo::randomPoints(mask = en.vir,
                                         n = more, ext = NULL, extf = 1.1, excludep = TRUE,
                                         prob = FALSE, cellnumbers = FALSE, tryf = 3,
                                         warn = 2, lonlatCorrection = TRUE)
        more.bak.vir <- raster::extract(en.vir, more.back)
        more.bak0 <- more.bak.vir[, colnames(more.bak.vir) %in%
                                    colnames(present.env0)]
      ...
      if (is.null(nrow(diff)) || nrow(diff) == 0) {
        ...
        ref.sd <- apply(present.env0, FUN = stats::sd,
                        MARGIN = 2)
        ref.range <- apply(present.env0, FUN = max, MARGIN = 2) -
          apply(present.env0, FUN = min, MARGIN = 2)
        for (rs in 1:length(ref.sd)) {
          if (ref.sd[rs] == 0) {
            ref.sd[rs] <- ref.mean[rs]/4
          }
          if (ref.range[rs] == 0) {
        ...
        for (en in 1:ncol(present.env0)) {
          tmp.left <- stats::runif(outputNum/2, min = (q.01[en] -
                                                         2 * ref.range[en]), max = q.01[en] - ref.range[en])
          tmp.right <- stats::runif(outputNum/2, min = q.99[en] +
                                      ref.range[en], max = q.99[en] + 2 * ref.range[en])
  ...
  present.env <- as.data.frame(apply(present.env, FUN = as.numeric,
                                     MARGIN = 2))
  absent.env <- cbind(Count = 0, absent.env0)
  absent.env <- as.data.frame(apply(absent.env, FUN = as.numeric,
                                    MARGIN = 2))
  Data <- as.data.frame(rbind(present.env, absent.env))
  Data$Count = as.numeric(Data$Count)

  ### Drop variables that are multicollinearity
  # Remove variables through variance inflation factor (VIF)
  vif_fun <- function(df){
    vifs <- car::vif(stats::lm(df[,1] ~ ., data = df))
    if (max(vifs) < 10) { break }
    highest <- c(names((which(vifs == max(vifs)))))
    df <- df[,-which(names(df) %in% highest)]
    return(df)
  }
  Data <- vif_fun(Data)
  vif.env <- colnames(Data)[-1]

  if (model == "RF") {
    mod <- randomForest::randomForest(Count ~ ., Data, ntree = 500,
                                      importance = TRUE, na.action = randomForest::na.roughfix)
    use <- c(rep(1, nrow(present.env)), rep(0, nrow(absent.env)))
    prb <- c(stats::predict(mod, present.env),
             stats::predict(mod, absent.env))
    ...
    if (file.exists(jar)) {
      ...
      stop(paste("Please insure that the maxent.jar file have been placed into\n",
                 system.file(package = "dismo"), "/java",
                 sep = ""))
    }
  }
  roc1 <- pROC::roc(use, prb, precent = T, auc = T, plot = F, quiet = T)
  SST <- pROC::coords(roc1, x = "best", transpose = T,
                      ret = c("specificity",  "sensitivity", "threshold", "accuracy"))
  NMB <- list()
  NMB$model <- mod
  NMB$SST <- SST
  NMB$SelectedVariables <- vif.env
  return(NMB)
}
